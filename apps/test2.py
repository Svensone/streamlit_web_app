...
def app():
    ...
    """
        ...
        with Grid("1 1 1", color='COLOR', background_color='') as grid:
            grid.cell(
                class_="a",
                grid_column_start=1,
                grid_column_end=4,
            ...
            # grid.cell("b", 1, 4, 2, 3).text("The cell to the left is a dataframe")
            # grid.cell("c", 3, 4, 2, 3).plotly_chart(get_plotly_fig())
            grid.cell("d", 1, 4, 2, 3).dataframe(get_dataframe())
            # grid.cell("e", 3, 4, 2, 2).markdown(
            #     "Try changing the **block container style** in the sidebar!"
            # )
            grid.cell("f", 1, 4, 3, 4).text(
    ...
    set_png_as_page_bg('assets/bg-side.jpg')
    ...
    """
        def pyplot(self, fig=None, **kwargs):
            ...
            plt.savefig(string_io, format="svg", figsize=(2, 2))
